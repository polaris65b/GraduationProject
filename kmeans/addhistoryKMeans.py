...
# 각 user_id에 대한 마지막 행을 선택하지 않고 모든 데이터를 유지합니다.
# df = df.sort_values('train_id').groupby('user_id', as_index=False).last() # 이 줄을 주석 처리합니다.
...
for i, feature in enumerate(features, 1):
    ...
    if feature == 'train_count':
        # train_count와 Grade에 대한 그래프에서만 모든 데이터를 선으로 연결
        sorted_patient_data = patient_data.sort_values(by='train_count')
        plt.plot(sorted_patient_data['train_count'], sorted_patient_data['grade'], '-o', c='red', label=f'User ID {user_id} Path')
        plt.scatter(sorted_patient_data['train_count'].iloc[-1], sorted_patient_data['grade'].iloc[-1], c='red', s=200, marker='*', edgecolors='black', label=f'User ID {user_id} Latest')
    else:
        ...
        plt.scatter(patient_data[feature], patient_data['grade'], c='red', s=200, marker='*', edgecolors='black', label=f'User ID {user_id} (Top {percentile:.2f}%) in Cluster')
