...
for i, feature in enumerate(features, 1):
   ...
   # 환자 데이터에 별표로 표시
    if not patient_data.empty and feature == 'train_count':
        # 입력된 user_id와 동일한 Grade를 가진 데이터 필터링
        same_grade_df = df[df['grade'] == patient_data.iloc[0]['grade']]
        # train_count 값의 분포 확인
        train_count_values = same_grade_df['train_count'].values
        # 입력된 user_id의 train_count 값
        user_train_count = patient_data.iloc[0]['train_count']
        # 상위 몇 %에 해당하는지 계산 (기존 값들보다 크거나 같은 경우의 수를 사용)
