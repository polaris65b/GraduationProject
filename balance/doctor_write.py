...
import joblib
# Form implementation generated from reading ui file 'doctor_write.ui'
#
...
class doctor_write(object):
    #내가 추가####################################################
    def __init__(self, main_window):
        self.MainWindow = main_window

    def return_doctor_id(self,commentID):
        ...
        cursor.execute("SELECT docter_id FROM coment WHERE ID = ?", (commentID,))
        row = cursor.fetchall()
        doctor_id = row[0][0]
        return doctor_id
    
    def insert_coment(self, commentID):
        # 사용자가 작성한 텍스트 가져오기
        text = self.textEdit.toPlainText()
        
        # 현재 날짜 가져오기
        current_date = self.fix_dateEdit.date()
        ...
        cursor.execute("UPDATE coment SET coment = ? WHERE ID = ?", (comment, commentID))
        conn.commit()
        ...
        QMessageBox.information(None, "코멘트 전송", "코멘트 전송이 완료되었습니다.")

        # 연결 종료
        conn.close()
        ssh.close()
    
    def return_user_id(self,commentID):
        # SSH로 서버에 접속해서 Python 스크립트 실행
        ssh = paramiko.SSHClient()
        ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        ssh.connect('192.168.0.40', username='user', password='1234')
        ...
        cursor.execute("SELECT user_id FROM coment WHERE ID = ?", (commentID,))
        row = cursor.fetchall()
        user_id = row[0][0]
        return user_id

    def setupUi(self, MainWindow, commentID):
        MainWindow.setObjectName("MainWindow")
        MainWindow.resize(1280, 720)
        self.centralwidget = QtWidgets.QWidget(MainWindow)
        self.centralwidget.setObjectName("centralwidget")
        self.fix_label9 = QtWidgets.QLabel(self.centralwidget)
        self.fix_label9.setGeometry(QtCore.QRect(-110, -10, 1280, 721))
        self.fix_label9.setStyleSheet("background-color:rgb(255, 255, 255);")
        self.fix_label9.setText("")
        self.fix_label9.setScaledContents(False)
        self.fix_label9.setObjectName("fix_label9")
        self.fix_label_image = QtWidgets.QLabel(self.centralwidget)
        self.fix_label_image.setGeometry(QtCore.QRect(55, 75, 41, 41))
        ...
        self.fix_label1_1.setGeometry(QtCore.QRect(530, 40, 231, 61))
        font = QtGui.QFont()
        font.setPointSize(-1)
        ...
        self.fix_label1_1.setStyleSheet("font-size:30px;\n"
"font-weight: bold;background-color: rgba(255, 255, 255, 0);")
        ...
        self.scrollArea.setGeometry(QtCore.QRect(370, 110, 541, 441))
        ...
        self.train_num2.setGeometry(QtCore.QRect(1055, 133, 41, 31))
        font = QtGui.QFont()
        font.setFamily('Noto Sans KR Medium')
        font.setPointSize(-1)
        ...
        self.fix_label4_3.setStyleSheet("font-size:25px;background-color: rgba(255, 255, 255, 0);")
...
"")
        ...
        self.textEdit.setGeometry(QtCore.QRect(370, 570, 461, 91))
...
"border-radius:10px")
        ...
        self.write_button.setGeometry(QtCore.QRect(840, 570, 71, 91))
        font = QtGui.QFont()
        font.setFamily("Noto Sans KR Medium")
        font.setPointSize(13)
        self.write_button.setFont(font)
        self.write_button.setObjectName("write_button")
        self.write_button.setStyleSheet("background-color:rgb(255, 255, 255);")

        
        #내가 추가###################################################
        self.write_button.clicked.connect(lambda: self.insert_coment(commentID))
        #내가 추가################################################
        ...
        #내가 추가함###############################################################################
        self.home_button.clicked.connect(lambda: self.move_to_docter_search(commentID))
        self.logout_button.clicked.connect(self.move_to_Login)
        #내가 추가함############################################################################

        self.label = QtWidgets.QLabel(self.centralwidget)
        self.label.setGeometry(QtCore.QRect(-30, 0, 1271, 720))
        self.label.setText("")
...
"background-color: rgba(157, 157, 157, 45)")
        ...
        self.groupBox_2.setGeometry(QtCore.QRect(30, 430, 203, 251))
        self.groupBox_2.setStyleSheet("background-color:rgb(238, 238, 238);\n"
"border-radius:20px;border:1px solid rgb(201,201,201);")
        self.groupBox_2.setTitle("")
        self.groupBox_2.setObjectName("groupBox_2")
        self.sex_button = QtWidgets.QPushButton(self.groupBox_2)
        self.sex_button.setGeometry(QtCore.QRect(40, 20, 121, 101))
        self.sex_button.setStyleSheet("background-color: rgba(255,255,255, 200);"
                               "border: none;"
                               "border-radius: 0;")
        self.sex_button.setIcon(QtGui.QIcon("images/man.jpg"))  # 이미지 설정
        self.sex_button.setIconSize(QtCore.QSize(121, 101))  # 이미지 크기 설정
        self.sex_button.setObjectName("sex_button")

        
        #내가 추가#############################################################################
        self.sex_button.clicked.connect(lambda: self.move_to_user_inform(commentID))
        #내가 추가################################################################################
...
"border:none;")
        ...
        self.grade_change_label.setStyleSheet("font-size:20px;\nborder:0px solid rgb(255,255,255);background-color: rgba(255,255,255, 0);"
"")
...
"border: 1px solid rgb(216, 216, 216);")
        ...
        self.height_radiobutton.setGeometry(QtCore.QRect(390, 60, 91, 21))
        font = QtGui.QFont()
        font.setFamily("Noto Sans KR Medium")
        font.setPointSize(12)
        ...
        self.age_radiobutton.setGeometry(QtCore.QRect(750, 60, 91, 21))
        font = QtGui.QFont()
        font.setFamily("Noto Sans KR Medium")
        font.setPointSize(12)
        ...
        self.weight_radiobutton.setGeometry(QtCore.QRect(450, 60, 91, 21))
        font = QtGui.QFont()
        font.setFamily("Noto Sans KR Medium")
        font.setPointSize(12)
        ...
        self.count_radiobutton.setGeometry(QtCore.QRect(820, 60, 91, 21))
        font = QtGui.QFont()
        font.setFamily("Noto Sans KR Medium")
        font.setPointSize(12)
        ...
        #############################################################
        self.train_result_label.clicked.connect(lambda: self.graph_setting(commentID, '훈련 결과'))
        self.kmeans_label.clicked.connect(lambda: self.graph_setting(commentID, '분포도 분석'))
        self.garo_label.clicked.connect(lambda: self.graph_setting(commentID, '가로 기울기 변화도'))
        self.sero_label.clicked.connect(lambda: self.graph_setting(commentID, '세로 기울기 변화도'))
        self.grade_change_label.clicked.connect(lambda: self.graph_setting(commentID, '훈련 등급 변화'))
        self.fix_dateEdit.dateChanged.connect(lambda: self.calinder_change(commentID))

        self.height_radiobutton.clicked.connect(lambda: self.update_kmeans(commentID, 'height'))
        self.age_radiobutton.clicked.connect(lambda: self.update_kmeans(commentID, 'age'))
        self.weight_radiobutton.clicked.connect(lambda: self.update_kmeans(commentID, 'BFP'))
        self.count_radiobutton.clicked.connect(lambda: self.update_kmeans(commentID, 'train_count'))
        ##############################################################

        self.fix_label9.raise_()
        self.label.raise_()
        ...
        self.retranslateUi(MainWindow, commentID)
        QtCore.QMetaObject.connectSlotsByName(MainWindow)

    def retranslateUi(self, MainWindow, commentID):
        _translate = QtCore.QCoreApplication.translate
        #doctor_name=self.return_name(docterid)
        #user_name=self.return_user_name(userid)
        MainWindow.setWindowTitle(_translate("MainWindow", "MainWindow"))
        self.fix_label1_1.setText(_translate("MainWindow", "전체 훈련 파악"))
        #self.train_num2.setText(_translate("MainWindow", "14"))
        self.fix_label4_3.setText(_translate("MainWindow", "회차 훈련"))
        self.write_button.setText(_translate("MainWindow", "전송"))
        self.user_label.setText(_translate("MainWindow", '의사이름'))
        self.name_label_2.setText(_translate("MainWindow", '환자이름'))
        ...
        self.label_setting(commentID)
        self.user_date(commentID)
        self.graph_setting(commentID, '훈련 결과')
        self.train_num2.currentIndexChanged.connect(lambda: self.graph_setting(commentID, '훈련 결과'))
    ...
    def label_setting(self, commentID):
        # SSH로 서버에 접속해서 Python 스크립트 실행
        ssh = paramiko.SSHClient()
        ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        ssh.connect('192.168.0.40', username='user', password='1234')
        ...
        cursor.execute("SELECT user_id, docter_id FROM coment WHERE ID = ?", (commentID,))
        row = cursor.fetchall()
        ...
        cursor.execute("SELECT Name, Birth, Sex, Height, BFP FROM user WHERE ID = ?", (user_id, ))
        row = cursor.fetchall()

        print(row)

        self.name_label_2.setText(row[0][0])

        birth_date = row[0][2]
        birth_str = row[0][1] + " (만 " + str(self.calculate_age(row[0][1])) + "세)"
        self.birth_label.setText(birth_str)
        height_str = str(row[0][3]) + " / " + str(row[0][4]) + "%"
        ...
        if row[0][2] == 2:
            ...
            self.sex_button.setStyleSheet("background-color: rgba(255,255,255, 200);"
                                          "border: none;"
                                          "border-radius: 0;")
            self.sex_button.setIcon(QtGui.QIcon("images/woman.jpg"))  # 이미지 설정
            self.sex_button.setIconSize(QtCore.QSize(121, 101))  # 이미지 크기 설정
            self.sex_button.setObjectName("sex_button")
            self.sex_button.clicked.connect(lambda: self.move_to_user_inform(commentID))

        cursor.execute("SELECT Name FROM docter WHERE ID = ?", (doctor_id,))
        row = cursor.fetchall()

        self.user_label.setText(row[0][0])

    # ID를 통해 이름 텍스트 파일에 입력
    def user_date(self, commentID):
        global count_list
        ...
        # 쿼리 실행
        cursor.execute("SELECT user_id, Date, coment FROM coment WHERE ID = ?", (commentID,))
        row = cursor.fetchall()
        print(row)
    ...
    # 캘린더 변경되었을 때
    def calinder_change(self, commentID):
        global count_list
    ...
    # 다른 label setting
    def graph_setting(self, commentID, selectLabel):
        global count_list

        if selectLabel == '훈련 결과':
            self.target_graph()
        elif selectLabel == '분포도 분석':
            self.kmeans_graph(commentID)
    ...
    # kmeans 그래프 그리기
    def kmeans_graph(self, commentID):
        self.scrollArea.takeWidget()
        ...
        self.update_kmeans(commentID, 'height')

    def update_kmeans(self, commentID, selected_feature):
        self.scrollArea.takeWidget()
        # 기존 위젯 제거
    ...
    def garo_graph(self):
        ...
        plot_widget_pitch.setLabel('left',
                                   '좌&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;좌우 변화량&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;우')
    ...
    def move_to_docter_search(self, commentID):
        try:
            ...
            doctor_id = self.return_doctor_id(commentID)
            print(docterid,userid)
            self.home_window.setupUi(self.MainWindow, doctor_id)  # home 클래스의 setupUi 메서드 호출
            self.MainWindow.show()
        except Exception as e:
            print("login to docter_search error : ", e)

    def move_to_user_inform(self,commentID):
        try:
            print("test")
            ...
            user_id = self.return_user_id(commentID)
            doctor_id = self.return_doctor_id(commentID)
            print(user_id, doctor_id)
            self.home_window.setupUi(self.MainWindow, doctor_id, user_id)  # home 클래스의 setupUi 메서드 호출
            self.MainWindow.show()
        except Exception as e:
    ...
    def move_to_Login(self, userID):
        try:
            ...
            print("doctor_write to login error : ", e)
        #내가 추가함###############################################################################
...
if __name__ == "__main__":
    ...
    ui = doctor_write(MainWindow)
    ui.setupUi(MainWindow,17)
    MainWindow.show()
    sys.exit(app.exec_())
